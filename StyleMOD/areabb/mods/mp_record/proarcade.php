...
if ($cas_score == 3)
{
	...
	if ($db->sql_numrows($result)>1 )
	{
		...
		$page_a_charger = $server_protocol . $server_name . $server_port . preg_replace('/^\/?(.*?)\/?$/', "\\1", trim($board_config['script_path'])). '/games.'.$phpEx.'?gid='.$gid;
		$message = sprintf($lang['record_battu_message'],$row[0]['username'],$row[0]['game_libelle'],$row[1]['score_game'],$row[0]['score_game'],'[url='.$page_a_charger.']','[/url]');
